...
**目标**：比赛限时 30 分钟，死亡后基本不可复活（复活需其他玩家求助，实际很少发生），可立即重开且不限制局数；按 30 分钟内累计合成 9 的数量排名。训练和部署最终优化单位墙钟时间的合成吞吐，而不是单局存活或首次得分。据历史排行榜实测（`docs/research_1.md`）：限时模式本大区人类最强 **115** 个，成绩整体正态分布、无离群点；非限时模式（可复活一次，清空超限列后继续）人类最强单局 1000+。目标吞吐 **≥150、争取 ≥200**；单局最多合成数量是另一个独立训练目标。
