# 基于持续探索 N-Tuple 查表价值与 Beam 搜索的"兔儿爷"游戏智能体

> 用强化学习攻克网易《一梦江湖》内置合成小游戏**"兔儿爷"**（兔王争霸赛：
> 6 列 × 7 高栈、3 合 1 升级、9 消失得分、每 4 步一轮随机掉落）。
> 核心路线：**从游戏源码提取真实掉落分布 → 启发式教师 / PUCT 自博弈蒸馏（已落地
> 网页实战）→ 持续探索（ε 下限 0.10）的共享 N-Tuple 查表价值 TD(λ) → Beam 搜索**。
...
当前最强是阶段五：共享 N-Tuple 查表价值函数（`ntuple-ext-eps`，5.6 MB、纯 CPU、
无神经网络、无树搜索）配 beam 搜索。给价值函数喂**宽分布**数据（ε 下限 0.10 的
持续探索）是唯一的规模杠杆——主线从 5M 续训到 **12M 步已收官，1-ply 每千步
47.45（峰值 52.01，起点 19.6，+142%）**。在无步数上限的同种子配对评测（32 seed）
中，8M 版打出**单局 72.1 个 9 / 平均存活 1026 步 / 单局最高 309 个 9**，对阶段四的
PUCT 部署模型（9.6 个 9 / 199 步）配对差 **+62.5 [+40.0, +88.0]**；λ 0.4 分支在
同段还领先主线约 +2，续训冲 12M 中，是下一个"可能反超"的候选。阶段四模型
（`real-h5-puct-r2`，71 万参数等变 Policy/Value 网络 + 显式随机 PUCT）仍是已
落地的网页版实战方案，实测**单局 346 步 / 27,940 分 / 21 个 9**，网页与本地表现
经统计检验不可区分。
...
启发式教师示范（128 局 / 22,565 步 / 887 个 9）
  ...
  → r1 自示范 48 局（511 个 9，单局最高 36）→ r2 ★网页部署模型
```
...
### 阶段五：N-Tuple 价值 + Beam 搜索（当前主线，最强）
...
2. **持续探索（ε 下限 0.10）是唯一有效杠杆**。同一 V、同一 λ，只把 ε 从 0.01
   抬到 0.10，1-ply 每千步 3M 步内从 19.6 涨到 39.4，一路爬到 12M 收官的 47.45
   （峰值 52.01）；ε 收敛到 0.01 的对照线同期只涨到 29.6 且后段趋平。ε 响应
   曲线（8M 口径）：`0.01→29.6，0.10→39.4，0.15→41.2，0.25→39.9`——**甜点在
   0.10 附近**，再往上抬只有 +5% 且在噪声内。这与阶段二"喂自身策略窄分布会
   退化"是同一失败模式：**给 V 喂宽分布数据才涨**。另外，8M 时曾误判"进入
   平台"，续训推翻——**判断平台至少要 2M 步以上**。
3. **λ 响应曲线非单调：0.4 > 0.6 >> 0.8**。同起点同 ε 单变量对照，8M 时
   λ0.4 最后 10 次均值 41.92 对 λ0.6 的 39.35（约 3-4σ），且优势随步数保持；
   λ0.8 在 1.8M 步就死平在 22.7，灾难性有害。学习率 α 0.08 无益，保持 0.05。
4. **`best.pt` 是按单次 eval 最高分挑的，有系统性虚高**。训练期 eval 噪声约
...
训练现状（每条线只动一个旋钮，续训线随时断电需 `--resume` 拉起）：

| run_id | 变量 | 进度 | 1-ply 最后 10 次均值 |
|---|---|---|---|
| `ntuple-ext-eps` | ε 0.10、λ 0.6 | **12.0M 收官** | **47.45**（峰值 52.01） |
| `ntuple-lam40` | λ 0.4 | 续训 12M 中（暂停 8.62M） | 43.46（同段领先主线 +2） |
| `ntuple-lam30` | λ 0.3 | 探曲线左侧（暂停 5.72M） | ~30，暂未见左侧优势 |
...
注意两个 caveat：① 72.12 个 9/局那组部署级证据是 **8M 版**测的，磁盘上的
`final.pt` 已换成 12M 权重（1-ply +20%，理应更强），**12M 无上限配对评测待跑**；
② `ntuple-lam40`（λ 0.4）续训至 12M 后可能反超主线，届时一并定夺。
尚未接入网页版与实机链路，接入时按新 V 重扫搜索深度。
...
7. **每局 9 数的方差 ≈ 均值（变异系数 100%）**，且几乎全部来自存活时长
   ...
   用于衡量部署能力。
8. **宽分布探索是价值函数续训的唯一燃料**：ε 下限抬到 0.10 一个旋钮 +101%，
   而加学习率、加深搜索、beam 行为策略自训全部无益或有害；抬高 ε 会先跌后爬，
   必须给够步数（≥2M）再下判断。
...
**模型下载**：已发布模型 `real-h5-puct-r2-pv.pt`（2.8 MB，自包含检查点）在
[Release v1.0.0](https://github.com/Gongcai/wyclx_game_ai/releases/tag/v1.0.0)，
放到 `runs/puct-v3/` 即可（或用 `--model` 指定路径）。当前最强的
`runs/ntuple-ext-eps/final.pt`（5.6 MB）随 `runs/` 保存于训练机，未随 release
发布。
...
# N-Tuple Network + afterstate TD(lambda)（当前主线，CPU 可跑）
# 关键配方：--eps-end 0.10（持续探索，漏了就退回慢线）；--trace-lambda 0.4
# 优于默认 0.6（0.8 灾难性有害）；α 保持 0.05。断点续训加 --resume <run_id>。
python train_ntuple.py --dist real --steps 12000000 --gamma 0.99 --trace-lambda 0.4 \
  --alpha 0.05 --eps-end 0.10 --merge-w 1.0 --n9-w 30 --death-w 15 --run-id ntuple-lam40
python eval_ntuple.py --model runs/ntuple-lam40/final.pt --dist real --n 1000
