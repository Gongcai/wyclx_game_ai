...
def heuristic_health(game, w=HeuristicWeights()):
    """评估盘面健康度，越高越好。game 为 move_afterstate 后的状态（可含第4步已应用预告）。"""
    if game.dead:
        return -w.death
...
def heuristic_policy(game, w=HeuristicWeights(), rng=None):
    ...
    第 4 步（预告已知）：move 后应用已知预告再评估健康，避免必然溢出死亡。
    前 1-3 步：评估 move 后 afterstate 健康（惩罚 7 高列，提前避免死亡陷阱）。
    """
    best_action = None
    ...
    for src, dst in game.legal_moves():
        ...
        if sim.moves % 4 == 0 and sim.preview is not None:
...
def heuristic_policy_beam(game, w=HeuristicWeights(), width=16, depth=4, seed=0):
    ...
    第 4 步：应用已知预告精确推演；前 1-3 步：评估 afterstate 健康 + 为未来
    第 4 步采样预告。累积即时合并增益 + 终态健康作为序列评分，返回首步动作。
    ...
    for _step in range(depth):
        ...
        for state, seq, cum in beam:
            ...
            for src, dst in state.legal_moves():
                ...
                gain = float(sum(e for e in sim.events))
                if sim.moves % 4 == 0 and sim.preview is not None:
                    sim.resolve_afterstate()          # 已知预告，精确
                elif sim.moves % 4 == 3:
                    fd = sim.future_drops(1)
                    if fd is not None:
                        sim.resolve_afterstate(list(fd[0]))   # 预知：真实未来掉落
                    else:
                        sim.resolve_afterstate()              # 随机：采样分布
                else:
                    sim.resolve_afterstate()
